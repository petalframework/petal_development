defmodule AppWeb.Petal.Components.Navbar do
  use AppWeb, :surface_component

  prop current_user, :any, default: nil
  prop active_page, :atom, required: true
  prop top_menu_links, :list, default: []
  prop user_menu_links, :list, default: []
  ...
  def render(assigns) do
    ~F"""
    <div
      class={"bg-white border-b border-gray-200 dark:bg-gray-900 dark:border-gray-800", @class}
      x-data="{ open: false }"
    >
        ...
        <div class="flex justify-between h-16">
          <div class="flex">
            <div class="flex items-center flex-shrink-0">
              <!-- Desktop only - show full logo -->
              <LiveRedirect class="hidden lg:block" to={Routes.public_landing_page_path(@socket, :index)}>
                <svg
                  class="w-auto h-9"
                  width="1073"
                  height="352"
                ...
                >
                  <g clip-path="url(#clip0)">
                    ...
                    />
                    <path
                      d="M150.218 87.9707C157.627 115.638 155.361 144.148 147.427 158.368C132.763 155.807 102.915 125.568 95.1636 99.6579C89.2008 79.7271 85.1519 40.6845 101.145 27.1283C114.355 30.9222 132.979 39.3588 150.218 87.9707Z"
                      fill="#7C3AED"
                    />
                    <path
                      d="M78.6045 138.201C109.927 146.052 130.228 160.946 138.817 174.781C129.466 186.363 88.5459 197.802 62.1273 192.016C41.8052 187.566 5.69711 172.174 1.59718 151.613C11.3211 141.9 45.1508 130.695 78.6045 138.201Z"
                      fill="#8C3CE1"
                    />
                    <path
                      d="M80.7489 218.825C106.031 195.6 130.441 192.123 146.632 193.863C150.009 208.361 133.851 247.658 113.035 264.924C97.0237 278.207 62.6737 297.201 43.9427 287.783C42.2687 274.142 50.8591 244.328 80.7489 218.825Z"
                      fill="#9C3ED6"
                    />
                    <path
                      d="M152.192 279.216C152.725 239.013 156.505 222.969 166.703 210.273C180.733 215.248 205.111 250.048 208.425 276.889C210.975 297.536 208.446 336.706 190.413 347.401C178.022 341.453 152.139 310.034 152.192 279.216Z"
                      fill="#AD40C9"
                    />
                    <path
                    ...
                    />
                    <path
                      d="M277.344 215.12C251.989 205.366 242.705 199.213 237.047 189.776C243.486 182.049 271.251 174.749 289.039 178.934C302.722 182.154 326.955 192.931 329.51 206.864C322.839 213.324 303.269 220.84 277.344 215.12Z"
                      fill="#CB44B2"
                    />
                    <path
                      d="M299.851 164.347C279.962 161.477 272.256 158.563 266.625 152.687C270 146.056 288.826 136.231 302.343 136.328C312.741 136.403 331.989 140.195 336.12 149.825C332.369 155.58 319.489 164.215 299.851 164.347Z"
                      fill="#D445AB"
                    />
                  </g>
              ...
              <!-- Mobile only - show logo as icon -->
              <LiveRedirect class="block lg:hidden" to={Routes.public_landing_page_path(@socket, :index)}>
                <img
                  class="w-auto h-8"
            ...
            <!-- Desktop only - top menu items -->
            <div class="hidden space-x-6 sm:-my-px sm:ml-10 sm:flex">
              <LiveRedirect
                :for={item <- @top_menu_links}
                to={item.path}
                label={item.label}
                class={if @active_page == item.name, do: active_class(), else: inactive_class()}
              />
            </div>
          </div>
          ...
          <div class="flex items-center">
            ...
            <div :if={@current_user} class="hidden sm:flex sm:items-center">
              <div @click.outside="open = false" class="relative z-10 ml-3" x-data="{ open: false }">
                <div>
                  <button
                    @click="open = !open"
                  ...
                  >
                    <Avatar {...avatar_props_for_user(assigns[:current_user])} />
                  </button>
                </div>
                <div
                  {...%{
                ...
                >
                  ...
                  >
                    <LiveRedirect
                      :for={item <- @user_menu_links}
                      to={item.path}
                      label={item.label}
                      class={if @active_page == item.name, do: dropdown_active_class(), else: dropdown_inactive_class()}
                    />
                  </div>
                </div>
              </div>
            ...
            <div class="flex items-center ml-2 -mr-2 sm:hidden">
              <button
                @click="open = !open"
      ...
      <!-- Mobile only - menu dropdown when menu toggle pressed -->
      <div {%{":class" => "{ 'block': open, 'hidden': !open }"}} class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
          <LiveRedirect
            ...
            to={item.path}
            label={item.label}
            class={if @active_page == item.name, do: mobile_active_class(), else: mobile_inactive_class()}
          />
        </div>

        <div :if={@current_user} class="pt-4 pb-3 border-t border-gray-200">
          <div class="flex items-center px-4 space-x-3">
            <div class="flex-shrink-0">
              <Avatar {...avatar_props_for_user(@current_user)} />
            </div>
            <div>
              <div class="text-base font-medium leading-6 text-gray-800">{@current_user.name}</div>
          ...
          <div class="mt-3 space-y-1" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
            <LiveRedirect
              ...
              to={item.path}
              label={item.label}
              class={if @active_page == item.name, do: inner_menu_active_class(), else: inner_menu_inactive_class()}
            />
          </div>
        </div>
      </div>
  ...
  def inactive_class,
    do: "inline-flex items-center
      px-1 pt-1
      border-b-2
      border-transparent
      text-sm font-medium leading-5
      text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:text-gray-700 focus:border-gray-300
      dark:hover:text-gray-300 dark:hover:border-gray-700 dark:focus:text-gray-300 dark:focus:border-gray-700
  ...
  def mobile_inactive_class,
    do: "block pl-3 pr-4 py-2
  ...
  def inner_menu_inactive_class,
    do:
      "block px-4 py-2 text-base font-medium text-gray-500 transition duration-150 ease-in-out hover:text-gray-800 hover:bg-gray-100 focus:outline-none focus:text-gray-800 focus:bg-gray-100"

  def inner_menu_active_class,
    do:
      "block px-4 py-2 text-base font-medium transition duration-150 ease-in-out text-gray-800 bg-gray-100 focus:outline-none"

  def dropdown_active_class,
    do: "block px-4 py-2 text-sm text-gray-700 bg-gray-100 transition duration-150 ease-in-out"

  def dropdown_inactive_class,
    do:
      "block px-4 py-2 text-sm text-gray-700 transition duration-150 ease-in-out hover:bg-gray-100"
end
